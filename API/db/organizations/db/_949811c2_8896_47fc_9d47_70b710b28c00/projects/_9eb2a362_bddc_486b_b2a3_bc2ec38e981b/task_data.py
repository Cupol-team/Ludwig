...
class TaskData(SqlAlchemyProjBase, SerializerMixin):
    ...
    type = sqlalchemy.Column(UUID(as_uuid=True), nullable=True, primary_key=True)
    priority = sqlalchemy.Column(sqlalchemy.Integer, primary_key=True)
    status = sqlalchemy.Column(UUID(as_uuid=True), nullable=True, primary_key=True)
