...
const TaskDetailsModal = ({ taskUuid, onClose, avatars }) => {
  ...
  const getStatusClass = (statusName) => {
    ...
    if (lowerCaseStatus.includes('нов') || lowerCaseStatus.includes('new')) {
      return 'status-new';
    } else if (
      lowerCaseStatus.includes('в процессе') || 
      lowerCaseStatus.includes('progress') || 
      lowerCaseStatus.includes('работе') || 
      lowerCaseStatus.includes('в работе')
    ) {
      return 'status-in-progress';
    } else if (lowerCaseStatus.includes('тест') || lowerCaseStatus.includes('test')) {
      return 'status-testing';
    } else if (
      lowerCaseStatus.includes('готов') || 
      lowerCaseStatus.includes('done') || 
      lowerCaseStatus.includes('завершен')
    ) {
      return 'status-done';
    } else if (lowerCaseStatus.includes('блок') || lowerCaseStatus.includes('block')) {
              ...
              <div className="modal-body">
                <div className="modal-left">
                  {taskDetail.description ? (
                    ...
                    <div className="section">
                      <h3>Описание</h3>
                      <p className="empty-description">Нет описания</p>
                    </div>
                  )}
                </div>
                <div className="modal-right">
                  <div className="section">
                    <h3>Статус</h3>
                    <div className={`task-status ${getStatusClass(taskDetail.statusName)}`}>
                      {taskDetail.statusName || 'Нет статуса'}
                    </div>
                  </div>
                  <div className="section">
                    <h3>Приоритет</h3>
                  ...
                  <div className="section">
                    <h3>Тип</h3>
                    <div className="task-type">{taskDetail.typeName || 'Нет типа'}</div>
                  </div>
                  <div className="section">
                    <h3>Срок</h3>
                    <div className="task-date">
                      {taskDetail.date
                        ? new Date(taskDetail.date).toLocaleDateString('ru-RU', {
                            day: '2-digit',
                            month: '2-digit',
                            year: 'numeric'
                          })
                  ...
                  <div className="section">
                    ...
                    <div className="executors-list">
                      {taskDetail.executors && taskDetail.executors.length > 0 ? (
                        taskDetail.executors.map((executorId, index) => (
